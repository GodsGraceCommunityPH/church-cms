...
declare
  ...
  corrected_requirements jsonb := '[]'::jsonb;
...
begin
  ...
  if p_attendance_status is not null and p_attendance_status not in ('present','late','absent','excused') then
  ...
  where id=p_enrollment_id and archived_at is null for update;
  if not found then raise exception 'Training enrollment not found' using errcode='P0002'; end if;

  select * into target_session from public.training_sessions where id=p_session_id for update;
  ...
  select status into class_status from public.training_batches where id=enrollment.batch_id;
  ...
  select status into previous_attendance from public.training_attendance
  where member_training_id=p_enrollment_id and session_id=p_session_id;

  select coalesce(jsonb_agg(jsonb_build_object(
    'requirement_id', requirement.id,
    'name', requirement.name,
    'completed', coalesce(progress.completed,false)
  ) order by requirement.display_order), '[]'::jsonb)
  ...
  where requirement.training_id=enrollment.training_id
    and (requirement.is_active or progress.id is not null);

  if p_attendance_status is null then
    delete from public.training_attendance
    where member_training_id=p_enrollment_id and session_id=p_session_id;
  else
    insert into public.training_attendance(member_training_id,session_id,status,recorded_by,recorded_at)
    values(p_enrollment_id,p_session_id,p_attendance_status,auth.uid(),now())
    on conflict(member_training_id,session_id) do update
      set status=excluded.status,recorded_by=auth.uid(),recorded_at=now(),updated_at=now();
  end if;
  ...
  for requirement_item in select value from jsonb_array_elements(p_requirements) loop
    ...
    if not exists(select 1 from public.training_program_requirements
      where id=requirement_id and training_id=enrollment.training_id) then
      raise exception 'Requirement does not belong to this Training program' using errcode='P0001';
    end if;
    if requirement_completed then
      insert into public.member_training_session_requirement_progress(
        member_training_id,training_session_id,program_requirement_id,completed,completed_at,completed_by
      ) values(p_enrollment_id,p_session_id,requirement_id,true,now(),auth.uid())
      ...
      where member_training_id=p_enrollment_id and training_session_id=p_session_id
  ...
  insert into public.member_training_workflow_events(
    member_training_id,event_type,previous_status,new_status,previous_batch_id,new_batch_id,
    reason,acted_by,snapshot
  ) values(
    enrollment.id,'student_record_corrected',enrollment.workflow_status,enrollment.workflow_status,
    enrollment.batch_id,enrollment.batch_id,btrim(p_reason),auth.uid(),
    jsonb_build_object(
      'session_id',target_session.id,'session_title',target_session.title,
      'previous_attendance',previous_attendance,'new_attendance',p_attendance_status,
      'previous_requirements',previous_requirements,'new_requirements',corrected_requirements
    )
  );
...
revoke all on function public.correct_completed_student_session_record(uuid,uuid,text,jsonb,text) from public,anon;
grant execute on function public.correct_completed_student_session_record(uuid,uuid,text,jsonb,text) to authenticated;
