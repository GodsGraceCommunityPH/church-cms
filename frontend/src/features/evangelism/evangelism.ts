...
export interface EvangelismContact { id:string; firstName:string; lastName:string; mobile:string; normalizedMobile:string; addressArea:string; dateReached:string; reachedBy:string; source:EvangelismSource; sourceOther:string; status:EvangelismStatus; notes:string; memberId:string|null; createdAt:string; updatedAt:string; }
export type EvangelismInput=Omit<EvangelismContact,'id'|'normalizedMobile'|'memberId'|'createdAt'|'updatedAt'>;
...
export const blankEvangelismContact=():EvangelismInput=>({firstName:'',lastName:'',mobile:'',addressArea:'',dateReached:new Date().toLocaleDateString('en-CA'),reachedBy:'',source:'personal_evangelism',sourceOther:'',status:'new',notes:''});
